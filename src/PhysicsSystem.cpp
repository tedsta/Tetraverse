...
void PhysicsSystem::processEntity(Entity *entity, const float dt)
{
    ...
    bool isGrid = entity->getComponent(GridComponent::Type)!=NULL;
    ...
    if (!isGrid)
    {
        ...
        {
            float fix = 0;
            if (grid->checkCollision(trans, sf::Vector2f(phys->getWidth(), phys->getHeight()), vdir, fix))
            {
                phys->mVelocity.y = 0;
    ...
    {
        trans->move(sf::Vector2f(phys->mVelocity.x, phys->mVelocity.y)*dt);
    }
}
